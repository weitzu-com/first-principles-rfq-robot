...
# ================== State definition: six-step RFQ workflow ==================
...
def snov_find_contact(domain: str) -> Dict:
    ...
    try:
        ...
        emails = search_response.json().get("data", {}).get("emails", [])
        for entry in emails:
...
def analyze_physical_requirements(state: RFQState) -> RFQState:
    ...
    fallback = {
        "summary": specs[:500],
        "material": _match_or_default(specs, ["aluminum", "steel", "titanium", "copper", "plastic", "composite"]),
        "process": _match_or_default(specs, ["cnc", "machining", "sheet metal", "casting", "forging", "3d print", "welding"]),
        "quantity": _extract_quantity(specs),
        "certifications": _find_terms(specs, ["AS9100", "ISO 9001", "ITAR", "NADCAP"]),
        "risk_flags": _find_terms(specs, ["tight tolerance", "critical", "flight", "pressure", "thermal", "safety"]),
    }
    prompt = (
        ...
        packages.append(
            {
                ...
                "body": _draft_rfq_body(state, supplier),
            }
        )
...
def recommend_next_step(state: RFQState) -> RFQState:
    ...
    if state.get("in_house_decision"):
        ...
        recommendation = f"Send RFQs to the top {rfq_count} qualified supplier(s), then compare lead time, certification fit, and NRE/tooling costs."
    else:
        recommendation = (
...
def _match_or_default(text: str, terms: List[str]) -> str:
    lowered = text.lower()
    for term in terms:
...
def _extract_quantity(text: str) -> str:
    match = re.search(r"\b(?:qty|quantity|数量)\s*[:=]?\s*(\d+[,\d]*)", text, re.IGNORECASE)
    if match:
        return match.group(1)
    match = re.search(r"\b(\d+[,\d]*)\s*(?:pcs|pieces|units|件)\b", text, re.IGNORECASE)
    return match.group(1) if match else "unknown"


def _draft_rfq_body(state: RFQState, supplier: Dict[str, Any]) -> str:
    requirements = state.get("physical_requirements", {})
    contact = supplier.get("contact", {})
    return (
        ...
        f"Certifications: {', '.join(requirements.get('certifications', []) or []) or 'not specified'}\n\n"
...
# ================== Streamlit UI ==================
def main() -> None:
    st.set_page_config(page_title="First Principles RFQ Robot", page_icon="🛠️", layout="wide")
...
def render_result(result: RFQState) -> None:
    ...
    for package in result.get("rfq_results", []):
        with st.expander(package.get("supplier", "Supplier")):
            ...
            st.text_input("Subject", package.get("subject", ""), key=f"subject-{package.get('supplier', '')}")
            st.text_area("RFQ body", package.get("body", ""), height=260, key=f"body-{package.get('supplier', '')}")
