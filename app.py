...
def analyze_physical_requirements(state: RFQState) -> RFQState:
    ...
    fallback = {
        "summary": specs[:500],
        "material": match_or_default(
            specs,
            ["aluminum", "steel", "titanium", "copper", "plastic", "composite"],
        ),
        "process": match_or_default(
            specs,
            ["cnc", "machining", "sheet metal", "casting", "forging", "3d print", "welding"],
        ),
    ...
    return {"physical_requirements": {**fallback, **(parsed or {})}}
...
def search_candidate_suppliers(state: RFQState) -> RFQState:
    ...
    query = " ".join(query_parts).strip() or state["part_specs"][:120]
...
def recommend_next_step(state: RFQState) -> RFQState:
    ...
    if state.get("in_house_decision"):
        ...
        recommendation = (
            ...
            "certification fit, and tooling costs."
        )
    else:
        recommendation = (
...
def draft_rfq_body(state: RFQState, supplier: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"Certifications: {', '.join(certifications) if certifications else 'not specified'}\n\n"
...
def main() -> None:
    st.set_page_config(page_title="First Principles RFQ Robot", layout="wide")
    ...
    with st.sidebar:
        ...
        snov_configured = bool(os.getenv("SNOVIO_CLIENT_ID") and os.getenv("SNOVIO_CLIENT_SECRET"))
        st.write(f"Snov.io: {'configured' if snov_configured else 'missing SNOVIO_CLIENT_ID / SNOVIO_CLIENT_SECRET'}")

    part_specs = st.text_area(
        "Part specifications",
        height=220,
        placeholder="Example: Qty 50 CNC machined 7075 aluminum bracket, AS9100 supplier preferred...",
    )
